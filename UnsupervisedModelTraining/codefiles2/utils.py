...
def sst_binary(data_dir='Stanford_Sentiment_Data/'):
    ...
    folder.
    """
    X, Y = load_sst(os.path.join(data_dir, 'test_binary_sent.csv'))
    #vaX, vaY = load_sst(os.path.join(data_dir, 'test_binary_sent.csv'))
    #teX, teY = load_sst(os.path.join(data_dir, 'test_binary_sent.csv'))
    """
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2)
    for train_idx, test_idx in sss.split(X, Y):
        #print(train_idx, test_idx)
        trX = [X[i] for i in train_idx] 
        trY = [Y[i] for i in train_idx] 
        teX = [X[i] for i in test_idx] 
        teY = [Y[i] for i in test_idx] 
    ...
    trX, trY = load_sst(os.path.join(data_dir, 'datafinal.csv'))
    vaX, vaY = load_sst(os.path.join(data_dir, 'datafinal.csv'))
    teX, teY = load_sst(os.path.join(data_dir, 'datafinal.csv'))
    """
    trX = X
    trY = Y
    teX = X
...
def preprocess(text, front_pad='\n ', end_pad=' '):
    ...
    #text =  np.array(bytearray(text), dtype='int32')
    return text
...
def concatenate_for_encoder(checkpoint_path):
    ...
    weights_list = np.load(checkpoint_path, allow_pickle=True, encoding='latin1')
